...
function buildCommand(args) {
    ...
    if (args.no_ignore_parent)
        cmd.push('--no-ignore-parent');
    if (args.no_ignore_vcs)
        cmd.push('--no-ignore-vcs');
    if (args.hidden)
        cmd.push('--hidden');
    if (args.no_heading)
        cmd.push('--no-heading');
    ...
    if (args.require_git)
        cmd.push('--require-git');
    if (args.pass_through)
        cmd.push('--passthru');
    if (args.auto_hybrid_regex)
        cmd.push('--auto-hybrid-regex');
    if (args.hyper_scan)
        cmd.push('--hyper-scan');
    if (args.pcre2)
        cmd.push('--pcre2');
    if (args.no_pcre2_unicode)
        cmd.push('--no-pcre2-unicode');
    if (args.no_unicode)
        cmd.push('--no-unicode');
    if (args.no_encoding)
        cmd.push('--no-encoding');
    if (args.no_mmap)
        cmd.push('--no-mmap');
    if (args.no_pre)
        cmd.push('--no-pre');
    if (args.no_jit)
        cmd.push('--no-jit');
    if (args.threads)
        cmd.push('--threads', args.threads.toString());
    if (args.depth)
        cmd.push('--depth', args.depth.toString());
    if (args.min_depth)
        cmd.push('--mindepth', args.min_depth.toString());
    if (args.max_depth)
        cmd.push('--maxdepth', args.max_depth.toString());
    if (args.glob_case_insensitive)
        cmd.push('--glob-case-insensitive');
    if (args.no_glob)
        cmd.push('--no-glob');
    if (args.no_pre_glob)
        cmd.push('--no-pre-glob');
    if (args.no_post_glob)
        cmd.push('--no-post-glob');
    if (args.no_pre_grep_glob)
        cmd.push('--no-pre-grep-glob');
    if (args.no_post_grep_glob)
        cmd.push('--no-post-grep-glob');
    if (args.mmap)
        cmd.push('--mmap');
    if (args.encoding)
        cmd.push('--encoding', args.encoding);
    if (args.context !== undefined)
        cmd.push('-C', args.context.toString());
    if (args.before_context !== undefined)
        cmd.push('-B', args.before_context.toString());
    if (args.after_context !== undefined)
        cmd.push('-A', args.after_context.toString());
    if (args.max_columns !== undefined)
        cmd.push('--max-columns', args.max_columns.toString());
    if (args.max_filesize)
        cmd.push('--max-filesize', args.max_filesize);
    if (args.max_count !== undefined)
        cmd.push('--max-count', args.max_count.toString());
    ...
    if (args.type)
        cmd.push('-t', args.type);
    if (args.iglob)
        cmd.push('--iglob', args.iglob);
    if (args.glob)
        cmd.push('--glob', args.glob);
    if (args.pre)
        cmd.push('--pre', args.pre);
    if (args.post)
        cmd.push('--post', args.post);
    if (args.pre_glob)
        cmd.push('--pre-glob', args.pre_glob);
    if (args.post_glob)
        cmd.push('--post-glob', args.post_glob);
    if (args.pre_grep)
        cmd.push('--pre-grep', args.pre_grep);
    if (args.post_grep)
        cmd.push('--post-grep', args.post_grep);
    if (args.pre_grep_glob)
        cmd.push('--pre-grep-glob', args.pre_grep_glob);
    if (args.post_grep_glob)
        cmd.push('--post-grep-glob', args.post_grep_glob);
    if (args.field_context_separator)
        cmd.push('--field-context-separator', args.field_context_separator);
    if (args.field_match_separator)
        cmd.push('--field-match-separator', args.field_match_separator);
    if (args.field_file_separator)
        cmd.push('--field-file-separator', args.field_file_separator);
    if (args.sort_files)
        cmd.push('--sort-files');
    if (args.sort_r)
        cmd.push('--sortr');
